...
class NNAgent:
    def __init__(self, config, restore_dir=None, device="cpu"):
        ...
        self.__pv_vector = tf.reduce_sum(self.__net.output * self.__future_price, axis=1) *\
                           (tf.concat([tf.ones(1), self.__pure_pc()], axis=0))
        self.__log_mean_free = tf.reduce_mean(tf.math.log(tf.reduce_sum(self.__net.output * self.__future_price,
                                                                   axis=1)))
        self.__portfolio_value = tf.reduce_prod(self.__pv_vector)
        self.__mean = tf.reduce_mean(self.__pv_vector)
        self.__log_mean = tf.reduce_mean(tf.math.log(self.__pv_vector))
    ...
    def __set_loss_function(self):
        def loss_function4():
            return -tf.reduce_mean(tf.math.log(tf.reduce_sum(self.__net.output[:] * self.__future_price,
                                                        axis=1)))

        def loss_function5():
            return -tf.reduce_mean(tf.math.log(tf.reduce_sum(self.__net.output * self.__future_price, axis=1))) + \
                   LAMBDA * tf.reduce_mean(tf.reduce_sum(-tf.math.log(1 + 1e-6 - self.__net.output), axis=1))
